...
TraceCare AI is a local competition prototype for traceable clinical evidence and event lifecycle demonstration. Sprint 0 uses synthetic patients and deterministic creatinine trend rules to create review-required events.
...
6. Open P001 detail.
7. Review `0.8 -> 1.3 mg/dL`, `RAPID_INCREASE`, and source documents.
8. Confirm simulated device state is `WARNING`.
9. Click `確認事件`; buzzer changes to off through `ACKNOWLEDGED`.
10. Click `標示為已處理`; event becomes `RESOLVED` and device state returns to `NORMAL`.
